...
`String` | Almost every type uses `Object.toString()` except for `Collections` which are joining their elements (converted to String) separated with `,` and `Enum` which is leveraging `Enum.name()` (not `Enum.toString()`). All types supported since HTL Runtime 1.2.6-1.4.0 ([SLING-9968](https://issues.apache.org/jira/browse/SLING-9968)).| [`ObjectModel.toString(Object)`](https://github.com/apache/sling-org-apache-sling-scripting-sightly-runtime/blob/org.apache.sling.scripting.sightly.runtime-1.2.4-1.4.0/src/main/java/org/apache/sling/scripting/sightly/render/ObjectModel.java#L238)
